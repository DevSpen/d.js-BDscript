module.exports = {
    name: "$isMuted",
    description: "Returns whether or not a user is muted. Applies to both self and server mutes.",
    returns: "?boolean",
    brackets: true,
    optional: true,
    fields: [{
    ...
    execute: async d => {
        if (!d.value.inside) {
            return d.deflate(d.message?.member?.voice.mute ?? "")
        }
        ...
        return d.deflate(member.voice.mute)
    }
}
