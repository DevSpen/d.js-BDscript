module.exports = {
    name: "$isVideoOn",
    description: "Returns whether or not a user has their video enabled in VC.",
    returns: "boolean",
    brackets: true,
    optional: true,
    fields: [{
    ...
    execute: async d => {
        if (!d.value.inside) {
            return d.deflate(d.message?.member?.voice.selfVideo ?? "")
        }
        ...
        return d.deflate(member.voice.selfVideo)
    }
}
