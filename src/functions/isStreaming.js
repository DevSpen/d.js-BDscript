module.exports = {
    name: "$isStreaming",
    description: "Returns whether or not a user is streaming in VC using Go Live.",
    returns: "boolean",
    brackets: true,
    optional: true,
    fields: [{
    ...
    execute: async d => {
        if (!d.value.inside) {
            return d.deflate(d.message?.member?.voice.streaming ?? "")
        }
        ...
        return d.deflate(member.voice.streaming)
    }
}
